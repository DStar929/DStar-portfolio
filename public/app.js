...
let authToken = sessionStorage.getItem('dstar_admin_token') || '';
...
document.getElementById('loginForm').addEventListener('submit', async e => {
  ...
  try {
    ...
    authToken = out.token;
    sessionStorage.setItem('dstar_admin_token', authToken);
...
async function logout() {
  if (authToken) {
    try { await fetch('/api/logout', {method:'POST', headers:{Authorization:'Bearer ' + authToken}}); } catch (_) {}
  }
  authToken = '';
...
async function removeProject(id) {
  ...
  const r = await fetch('/api/projects/' + encodeURIComponent(id), {method:'DELETE', headers:{Authorization:'Bearer ' + authToken}});
